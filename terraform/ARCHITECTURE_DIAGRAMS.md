# Architecture Diagrams

## 🏗️ Infrastructure Architecture

```
┌─────────────────────────────────────────────────────────────────────┐
│                          AWS Cloud (Region)                          │
│                                                                       │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │                    VPC (Your VPC ID)                         │   │
│  │                                                               │   │
│  │  ┌──────────────────────────────────────────────────────┐  │   │
│  │  │         Public Subnet (Your Subnet ID)                │  │   │
│  │  │                                                        │  │   │
│  │  │  ┌─────────────────────────────────────────────┐     │  │   │
│  │  │  │    Security Group (Your SG ID)              │     │  │   │
│  │  │  │                                              │     │  │   │
│  │  │  │  Inbound Rules:                             │     │  │   │
│  │  │  │  • Port 22  (SSH)   ← Your IP               │     │  │   │
│  │  │  │  • Port 80  (HTTP)  ← 0.0.0.0/0             │     │  │   │
│  │  │  │  • Port 443 (HTTPS) ← 0.0.0.0/0             │     │  │   │
│  │  │  │                                              │     │  │   │
│  │  │  │  ┌────────────────────────────────────┐    │     │  │   │
│  │  │  │  │  EC2 Instance (Ubuntu 22.04)       │    │     │  │   │
│  │  │  │  │  Type: t2.micro / t3.small         │    │     │  │   │
│  │  │  │  │  Storage: 20GB EBS (encrypted)     │    │     │  │   │
│  │  │  │  │  Public IP: Auto-assigned          │    │     │  │   │
│  │  │  │  │                                     │    │     │  │   │
│  │  │  │  │  ┌──────────────────────────────┐ │    │     │  │   │
│  │  │  │  │  │  Nginx (Port 80)             │ │    │     │  │   │
│  │  │  │  │  │  • Serves Frontend           │ │    │     │  │   │
│  │  │  │  │  │  • Reverse Proxy to Backend  │ │    │     │  │   │
│  │  │  │  │  └──────────────────────────────┘ │    │     │  │   │
│  │  │  │  │                                     │    │     │  │   │
│  │  │  │  │  ┌──────────────────────────────┐ │    │     │  │   │
│  │  │  │  │  │  Frontend (React)            │ │    │     │  │   │
│  │  │  │  │  │  Location: /var/www/...      │ │    │     │  │   │
│  │  │  │  │  │  Built with: Vite            │ │    │     │  │   │
│  │  │  │  │  └──────────────────────────────┘ │    │     │  │   │
│  │  │  │  │                                     │    │     │  │   │
│  │  │  │  │  ┌──────────────────────────────┐ │    │     │  │   │
│  │  │  │  │  │  Backend (Node.js)           │ │    │     │  │   │
│  │  │  │  │  │  Port: 3000                  │ │    │     │  │   │
│  │  │  │  │  │  Service: systemd            │ │    │     │  │   │
│  │  │  │  │  │  Express API                 │ │    │     │  │   │
│  │  │  │  │  └──────────────────────────────┘ │    │     │  │   │
│  │  │  │  │                                     │    │     │  │   │
│  │  │  │  │  ┌──────────────────────────────┐ │    │     │  │   │
│  │  │  │  │  │  PostgreSQL Database         │ │    │     │  │   │
│  │  │  │  │  │  Port: 5432 (localhost)      │ │    │     │  │   │
│  │  │  │  │  │  Database: bmidb             │ │    │     │  │   │
│  │  │  │  │  │  User: bmi_user              │ │    │     │  │   │
│  │  │  │  │  └──────────────────────────────┘ │    │     │  │   │
│  │  │  │  └────────────────────────────────────┘    │     │  │   │
│  │  │  └─────────────────────────────────────────────┘     │  │   │
│  │  └──────────────────────────────────────────────────────┘  │   │
│  │                                                               │   │
│  │  ┌──────────────────────────────────────────────────────┐  │   │
│  │  │             Internet Gateway                          │  │   │
│  │  └──────────────────────────────────────────────────────┘  │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                       │
└─────────────────────────────────────────────────────────────────────┘
                                 │
                                 │
                            Internet
                                 │
                                 ▼
                        ┌─────────────────┐
                        │  End Users      │
                        │  (Web Browsers) │
                        └─────────────────┘
```

## 🔄 Request Flow

```
User Request → Internet → Internet Gateway → Public Subnet
                                                    │
                                                    ▼
                                            Security Group
                                              (Port 80)
                                                    │
                                                    ▼
                                          Nginx on EC2 :80
                                                    │
                         ┌──────────────────────────┴──────────────────────┐
                         │                                                  │
                         ▼                                                  ▼
                Static Files Request                            API Request (/api/*)
                (HTML, CSS, JS)                                             │
                         │                                                  │
                         ▼                                                  ▼
                Frontend Files                                    Reverse Proxy
                /var/www/bmi-health-tracker/                              │
                         │                                                  ▼
                         │                                      Backend :3000
                         │                                      (Express API)
                         │                                                  │
                         │                                                  ▼
                         │                                      PostgreSQL :5432
                         │                                      (bmidb database)
                         │                                                  │
                         │◄─────────────────────────────────────────────────┘
                         │
                         ▼
                   User Browser
```

## 📦 Terraform Module Structure

```
terraform/
│
├── Root Module
│   ├── main.tf           ─────►  Calls EC2 Module
│   ├── variables.tf      ─────►  Defines inputs
│   ├── outputs.tf        ─────►  Exposes outputs
│   └── backend.tf        ─────►  S3 state config
│
└── modules/ec2/
    ├── main.tf           ─────►  Creates EC2 instance
    ├── variables.tf      ─────►  Module inputs
    └── outputs.tf        ─────►  Module outputs
```

## 🔐 Security Layers

```
┌────────────────────────────────────────────────────────────────┐
│                      Security Layers                            │
├────────────────────────────────────────────────────────────────┤
│                                                                 │
│  Layer 1: AWS Security Group                                   │
│  • Controls inbound/outbound traffic                           │
│  • Stateful firewall                                           │
│  • Port restrictions                                           │
│                                                                 │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  Layer 2: Network Security (VPC)                               │
│  • Isolated network environment                                │
│  • Subnet-level controls                                       │
│  • Public/Private subnet separation                            │
│                                                                 │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  Layer 3: Instance Security                                    │
│  • IMDSv2 enforced                                             │
│  • Encrypted EBS volumes                                       │
│  • SSH key authentication                                      │
│  • Regular security updates                                    │
│                                                                 │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  Layer 4: Application Security                                 │
│  • Nginx security headers                                      │
│  • PostgreSQL password auth                                    │
│  • Systemd service isolation                                   │
│  • Environment variable secrets                                │
│                                                                 │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  Layer 5: Data Security                                        │
│  • Database user permissions                                   │
│  • Connection encryption                                       │
│  • State file encryption (S3)                                  │
│  • Backup and recovery                                         │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

## 🚀 Deployment Flow

```
┌─────────────────────────────────────────────────────────────────┐
│                    Terraform Deployment Flow                     │
└─────────────────────────────────────────────────────────────────┘
                            │
                            ▼
                    ┌───────────────┐
                    │ terraform init │
                    │ • Load modules │
                    │ • Init backend │
                    └───────┬────────┘
                            │
                            ▼
                    ┌───────────────┐
                    │ terraform plan │
                    │ • Parse config │
                    │ • Show changes │
                    └───────┬────────┘
                            │
                            ▼
                    ┌────────────────┐
                    │ terraform apply│
                    │ • Create EC2   │
                    │ • Assign IP    │
                    │ • Run userdata │
                    └───────┬────────┘
                            │
            ┌───────────────┴───────────────┐
            │                               │
            ▼                               ▼
    ┌──────────────┐              ┌────────────────┐
    │ EC2 Launched │              │ Userdata Runs  │
    │ • Instance   │              │ • Copy script  │
    │ • Storage    │              │ • Setup env    │
    │ • Network    │              │ • Run deploy   │
    └──────────────┘              └────────┬───────┘
                                           │
                        ┌──────────────────┴──────────────────┐
                        │                                      │
                        ▼                                      ▼
            ┌────────────────────┐              ┌───────────────────────┐
            │ Install Software   │              │ Configure Services    │
            │ • Node.js          │              │ • PostgreSQL setup    │
            │ • PostgreSQL       │              │ • Backend systemd     │
            │ • Nginx            │              │ • Nginx config        │
            └────────────────────┘              └───────────────────────┘
                        │                                      │
                        └──────────────────┬──────────────────┘
                                           │
                                           ▼
                                ┌──────────────────┐
                                │ Deploy App       │
                                │ • Backend build  │
                                │ • Frontend build │
                                │ • Start services │
                                └────────┬─────────┘
                                         │
                                         ▼
                                ┌─────────────────┐
                                │ Application     │
                                │ Ready! 🎉       │
                                │ http://<IP>     │
                                └─────────────────┘
```

## 🔄 Application Data Flow

```
┌─────────────────────────────────────────────────────────────────┐
│                BMI Health Tracker - Data Flow                    │
└─────────────────────────────────────────────────────────────────┘

User Input (Browser)
        │
        │ HTTP Request
        ▼
   Nginx :80 ──────────────┐
        │                  │
        │                  │ Static Files (HTML/CSS/JS)
        │                  │
        │ /api/* requests  ▼
        │              React App
        │              (Frontend)
        │                  │
        ▼                  │
Backend :3000              │
(Express API)              │
        │                  │
        │ SQL Queries      │
        ▼                  │
PostgreSQL :5432           │
(Database)                 │
        │                  │
        │ Query Results    │
        ▼                  │
Backend :3000              │
        │                  │
        │ JSON Response    │
        ▼                  │
   Nginx :80               │
        │                  │
        │ HTTP Response    │
        └──────────────────┘
                │
                ▼
        User Browser
        (Display Results)


Specific Example: Create BMI Record
────────────────────────────────────

1. User fills form → Frontend (React)
                            │
                            ▼
2. POST /api/measurements
   { height, weight, date }
                            │
                            ▼
3. Nginx forwards → Backend :3000
                            │
                            ▼
4. Calculate BMI
   Express validates data
                            │
                            ▼
5. INSERT INTO measurements
   PostgreSQL executes
                            │
                            ▼
6. Return row ID
   { id: 123, bmi: 24.5 }
                            │
                            ▼
7. JSON response → Frontend
                            │
                            ▼
8. Update UI with new record
   Show success message
```

## 📁 File System Layout on EC2

```
/
├── home/
│   └── ubuntu/
│       └── bmi-health-tracker/          ← Application root
│           ├── IMPLEMENTATION_AUTO.sh   ← Deployment script
│           ├── backend/
│           │   ├── .env                 ← Config (DB credentials)
│           │   ├── package.json
│           │   ├── node_modules/
│           │   ├── src/
│           │   │   ├── server.js        ← Entry point
│           │   │   ├── routes.js
│           │   │   ├── db.js
│           │   │   └── ...
│           │   └── migrations/
│           │       ├── 001_create_measurements.sql
│           │       └── 002_add_measurement_date.sql
│           └── frontend/
│               ├── package.json
│               ├── dist/                ← Build output (deployed)
│               └── src/
│                   ├── main.jsx
│                   ├── App.jsx
│                   └── components/
│
├── var/
│   ├── www/
│   │   └── bmi-health-tracker/          ← Nginx serves from here
│   │       ├── index.html
│   │       ├── assets/
│   │       │   ├── index-*.js
│   │       │   └── index-*.css
│   │       └── ...
│   │
│   └── log/
│       ├── cloud-init-output.log        ← Userdata execution log
│       ├── user-data.log                ← Custom userdata log
│       ├── bmi-deployment.log           ← Deployment script log
│       ├── bmi-backend.log              ← Backend service log
│       └── nginx/
│           ├── bmi-access.log           ← Nginx access log
│           └── bmi-error.log            ← Nginx error log
│
├── etc/
│   ├── nginx/
│   │   ├── nginx.conf
│   │   ├── sites-available/
│   │   │   └── bmi-health-tracker       ← Nginx site config
│   │   └── sites-enabled/
│   │       └── bmi-health-tracker       ← Symlink
│   │
│   ├── systemd/
│   │   └── system/
│   │       └── bmi-backend.service      ← Backend systemd service
│   │
│   └── postgresql/
│       └── ...
│
└── usr/
    └── share/
        └── postgresql/
```

## 🎯 Port Mapping

```
External World                     EC2 Instance
─────────────────                 ───────────────

Internet :80  ──────────►  Nginx :80 ──┐
                                        │
Internet :443 ──────────►  Nginx :443  │
     (future SSL)                       │
                                        ├─► /var/www/... (Static)
                                        │
                                        └─► Backend :3000 (API)
                                                    │
                                                    ▼
                                            PostgreSQL :5432
                                            (localhost only)


SSH :22       ──────────►  SSH :22
   (Your IP only)           (Key-based auth)
```

## 🏷️ Tagging Strategy

```
All Resources Tagged With:
┌─────────────────────────────────────┐
│ Project     = "BMI-Health-Tracker"  │
│ Environment = "dev"                  │
│ ManagedBy   = "Terraform"            │
│ Application = "BMI-Health-Tracker"   │
│ Tier        = "Full-Stack"           │
│ + Custom Tags (from variables)       │
└─────────────────────────────────────┘

Benefits:
• Cost allocation
• Resource grouping
• Automation
• Compliance tracking
• Easy filtering
```

## 📊 State Management

```
┌─────────────────────────────────────────────────────────────┐
│                  Terraform State Flow                        │
└─────────────────────────────────────────────────────────────┘

Local Machine                      AWS
──────────────                    ─────

terraform apply  ─────────►  Create Resources
       │                           │
       │                           ▼
       │                    EC2, Security Groups, etc.
       │
       ▼
Save State ───────────────►  S3 Bucket
                             • Encrypted
                             • Versioned
                             • Remote
                             • Shared

Next Run:
terraform plan   ◄─────────  Load State from S3
       │
       ▼
Compare desired vs actual
       │
       ▼
Show changes


State File Contains:
├── Resource IDs (EC2 instance ID, etc.)
├── Current configuration
├── Dependencies
├── Outputs
└── Sensitive data (DB passwords!)
    ⚠️  Keep secure!
```

---

These diagrams provide a complete visual understanding of the infrastructure! 🎨

---

---

## 🧑‍💻 Author
*Md. Sarowar Alam*  
Lead DevOps Engineer, Hogarth Worldwide  
📧 Email: sarowar@hotmail.com  
🔗 LinkedIn: https://www.linkedin.com/in/sarowar/

---
